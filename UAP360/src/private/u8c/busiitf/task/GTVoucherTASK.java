...
public class GTVoucherTASK implements nc.bs.pub.taskcenter.IBackgroundWorkPlugin {
	...
	public String executeTask(BgWorkingContext param) throws BusinessException {
		...
		String sql1="select vouchid" + " from arap_djzb " + " where djdl='ys' and djlxbm='"
				+strbilltype+"' and dr=0 and isnull(zyx1,'')!='' and dwbm='"+strPkCorp+"' and djrq='" + strDdate+"'";
		 ...
		 * + " from arap_djzb " + " where djdl='ys' and djlxbm='"
		 * +strbilltype+"' and dr=0 and isnull(zyx1,'')!='' and dwbm='"+strPkCorp+"'";
		 * //if (!(strDjbh==null)&&(!strDjbh.equals(null))&&(!strDjbh.isEmpty())){
		 * sql1+=" and djbh='"+strDjbh+"'"; //} //主表vo ArrayList<DJZBHeaderVO> vos
		 * =(ArrayList<DJZBHeaderVO>) getDao().executeQuery(sql1, new
		 * BeanListProcessor(DJZBHeaderVO.class));
		 * 
		 * //公司 String
		 * sql2="select unitcode,unitname from bd_corp where pk_corp='"+strPkCorp+"'";
		 * CorpVO corpVO=(CorpVO)getDao().executeQuery(sql2, new
		 * BeanProcessor(CorpVO.class)); TokenGetVO
		 * tokenGetVO=u8c.server.XmlConfig.getTokenGetVO(corpVO.getUnitcode());
		 * 
		 * 
		 * for(DJZBHeaderVO vo : vos){ vouchid=vo.getVouchid(); List<GTVoucher>
		 * gTVouchers=new ArrayList();
		 * sql2="select isnull(zyx3,'') zyx3 from arap_djfb where vouchid='"
		 * +vouchid+"' group by zyx3"; ArrayList<Zyx3> zyx3s =(ArrayList<Zyx3>)
		 * getDao().executeQuery(sql2, new BeanListProcessor(Zyx3.class)); //2023-03-01
		 * 合并号zyx3 for(Zyx3 cZyx3:zyx3s) {
		 * 
		 * GTVoucher gTVoucher=new GTVoucher(); List<GTVoucherItem> items=new
		 * ArrayList();
		 * 
		 * //gTVoucher.setFPZL(tokenGetVO.getGTFPZL()); //20230303 发票分类 zyx15
		 * sql2="select doccode,docname from bd_defdoc where pk_defdoclist in (select pk_defdoclist from bd_defdoclist where doclistcode='FPZL') and docname='"
		 * +vo.getZyx15()+"'"; FPFL fPFL=(FPFL)getDao().executeQuery(sql2, new
		 * BeanProcessor(FPFL.class)); gTVoucher.setFPZL("004"); if (fPFL!=null) {
		 ...
		 * + "from arap_djfb " + "where vouchid='"
		 * +vouchid+"' and isnull(zyx13,'')!='success' and isnull(zyx3,'')='"+cZyx3.
		 * getZyx3()+"'"; ArrayList<DJZBItemVO> vobs =(ArrayList<DJZBItemVO>)
		 * getDao().executeQuery(sql2, new BeanListProcessor(DJZBItemVO.class)); //zyx3
		 * 合并号
		 * 
		 * for(nc.vo.ep.dj.DJZBItemVO vob : vobs){ //客户 String
		 * sql3="select custcode,custname,conaddr,phone1,taxpayerid,def1,def2,def3 from bd_cubasdoc where pk_cubasdoc='"
		 * +vob.getHbbm()+"'"; CustVO custVO=(CustVO)getDao().executeQuery(sql3, new
		 * BeanProcessor(CustVO.class));
		 * 
		 * //gTVoucher.setXTLSH(vob.getFb_oid()); //2023-03-01 合并号zyx3
		 ...
		 * 
		 * //20220303 //客户名称 zyx9 gTVoucher.setKHMC(vo.getZyx9()); //客户税号 zyx14
		 * gTVoucher.setKHSH(vo.getZyx14()); //客户开票地址电话 zyx10
		 * gTVoucher.setKHDZ(vo.getZyx10()); //客户银行账号 zyx13
		 * gTVoucher.setKHKHYHZH(vo.getZyx13()); //gTVoucher.setBZ(vob.getZyx2());
		 * 
		 * GTVoucherItem item=new GTVoucherItem();
		 * 
		 * item.setCPXH(""); item.setCPDW(""); item.setSL(vob.getSl().toString());
		 * item.setCPSL(""); item.setBHSJE(vob.getJfybwsje().toString());
		 * item.setSE(vob.getJfbbsj().toString());
		 * 
		 * //item.setFLBM(tokenGetVO.getGTFLBM()); //item.setCPMC(vob.getZyx2());
		 * 
		 * String zyx2="保险经纪费"; String flbm="304080299"; GTFLBM
		 * gTFLBM=u8c.server.XmlConfig.getGTFLBM(corpVO.getUnitcode(), vo.getDjlxbm());
		 * if (gTFLBM!=null) { strResult="税收分类:"+gTFLBM.getVal(); if
		 * ((!gTFLBM.getDef().equals(""))&&(gTFLBM.getDef().length()>0)&&(!gTFLBM.getDef
		 * ().isEmpty())){ zyx2=gTFLBM.getDef(); }else {
		 * zyx2=vob.getZyx2()+gTFLBM.getAdd(); } flbm=gTFLBM.getVal(); }
		 * item.setFLBM(flbm); item.setCPMC(zyx2);
		 * 
		 * item.setKCJE("0");
		 * 
		 * items.add(item); gTVoucher.setITEM(items);
		 * 
		 * }
		 * 
		 ...
		 * //Logger.debug("GTVouchers jsonBase64:"+strResult); Map<String, Object>
		 * headparams = new HashMap<String, Object>(); String
		 * strToken=getGTToken(tokenGetVO); headparams.put("token", strToken);
		 * //headparams.put("data", "["+strResult+"]"); headparams.put("spid",
		 * tokenGetVO.getGTSPID());
		 * Logger.debug("GTVouchers map:"+JSON.toJSONString(headparams)); String
		 * data="data="+"["+strResult+"]"; Logger.debug("GTVouchers data:"+data); try {
		 * strResult=HttpURLConnectionDemo.goldenTax(tokenGetVO.getGTFPKJUrl1(),
		 * tokenGetVO.getGTTokeContentType(), headparams, data);
		 * Logger.debug("GTVouchers result:"+JSON.toJSONString(strResult)); } catch
		 * (URISyntaxException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); Logger.error("GTVouchers error:"+e.getMessage()); }
		 * 
		 ...
		 * //sql1="update arap_djfb set zyx13='success',zyx14='发票代码:"+sucessList.getFpdm
		 * ()+"_发票号码:"+sucessList.getFphm()+"',zyx15='"+sucessList.getKprq()+"'"
		 * sql1="update arap_djfb set zyx13='success',zyx14='"+sucessList.getFpdm()+
		 * sucessList.getFphm()+"',zyx15='"+sucessList.getKprq()+"'"
		 * +" where vouchid+'_'+isnull(zyx3,'')='"+sucessList.getXtlsh()
		 * +"' and isnull(zyx13,'')!='success'"; try { getDao().executeUpdate(sql1); }
		 * catch (DAOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); Logger.error("GTVouchers success:"+e.getMessage()); }
		 * invoiceUpdateSet.updateInvoiceData(sucessList.getXtlsh(), param.getPk_user(),
		 * sucessList.getFpdm()+sucessList.getFphm(), sucessList.getKprq()); }
		 * for(ErrListVO errList:result.getErrList()){
		 * sql1="update arap_djfb set zyx13='err',zyx14='错误信息:"+errList.getErrMsg()+"'"
		 * +" where vouchid+'_'+isnull(zyx3,'')='"+errList.getXTLSH()
		 * +"' and isnull(zyx13,'')!='success'"; try { getDao().executeUpdate(sql1); }
		 * catch (DAOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); Logger.error("GTVouchers err:"+e.getMessage()); } }
		 * 
		 * 
		 * }
		 */
	...
	/*
	 * private String getGTToken(TokenGetVO tokenGetVO) { String strResult = "";
	 * Logger.init("hanglianAPI");
	 * 
	 * // strResult=JSON.toJSONString(tokenGetVO); Logger.debug("tokenGetVO:" +
	 * JSON.toJSONString(tokenGetVO)); String data = ""; Map<String, Object>
	 * headparams = new HashMap<String, Object>(); headparams.put("UserName",
	 * tokenGetVO.getGTUserName()); headparams.put("Password1",
	 * tokenGetVO.getGTPassword1()); try { strResult =
	 * HttpURLConnectionDemo.goldenTax(tokenGetVO.getGTTokeUrl(),
	 * tokenGetVO.getGTTokeContentType(), headparams, data); } catch
	 * (URISyntaxException e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); } Logger.debug("tokenGetResult:" + strResult); //
	 * JSONObject parameJson = JSONObject.parseObject(strResult); // TokenResult
	 * tokenResult=JSON.toJavaObject(parameJson,TokenResult.class); TokenResult
	 * tokenResult = JSONObject.parseObject(strResult, TokenResult.class); strResult
	 * = tokenResult.getID(); Logger.debug("tokenGetID:" + strResult); return
	 * strResult; }
	 * 
	 * private void updateInvoice(String strResult, String vouchid) { JSONObject
	 * parameJson = JSONObject.parseObject(strResult); InvoiceResultData
	 * arrivalResultData = JSON.toJavaObject(parameJson, InvoiceResultData.class);
	 * for (InvoiceResult invoiceResult : arrivalResultData.getInvoiceData()) { //
	 * String vouchid=invoiceResult.getInvoiceNo(); String sql =
	 * "update arap_djzb set zyx11='" + invoiceResult.getResultCode() + "',zyx12='"
	 * + invoiceResult.getResultDesc() + "'" + " where vouchid='" + vouchid + "'";
	 * try { getDao().executeUpdate(sql); } catch (DAOException e) { // TODO
