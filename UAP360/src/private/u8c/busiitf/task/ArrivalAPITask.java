...
public class ArrivalAPITask implements nc.bs.pub.taskcenter.IBackgroundWorkPlugin {
	...
	public String executeTask(BgWorkingContext param) throws BusinessException {
		...
		pk_user=param.getPk_user();
		
		if ((strDdate == null) || (strDdate.equals("")) || (strDdate.isEmpty())) {
			strDdate = strDate;
		}
		String sql1="select vouchid from arap_djzb " + " where isnull(zyx11,'')!='0' and djrq='"
		  +strDdate+"' and djdl='sk' and djlxbm='"+strbilltype+"' and dr=0 and dwbm='" +strPkCorp+"'"; 
		...
		ArrivalAPISet arrivalAPISet=new ArrivalAPISet();
		
		for (TaskParameterVO tpvo:taskParameterVOs) {
			strResult=arrivalAPISet.uploadBusiSys(tpvo.getVouchid(), strPkCorp, pk_user, strDdate);
		}
		/*
		 * String
		 ...
		 * + " from arap_djzb " + " where isnull(zyx11,'')!='0' and djrq='"
		 * +strDdate+"' and djdl='sk' and djlxbm='"+strbilltype+"' and dr=0 and dwbm='"
		 * +strPkCorp+"'"; if
		 * (!(strDjbh==null)&&(!strDjbh.equals(null))&&(!strDjbh.isEmpty())){
		 * sql1+=" and djbh='"+strDjbh+"'"; }
		 * 
		 * //初始化报文 Arrival arrival=new Arrival(); ArrivalHeader arrivalHeader=new
		 * ArrivalHeader(); arrivalHeader.setUserName("username");
		 * arrivalHeader.setPassWord("password");
		 * 
		 * arrivalHeader.setRequestDate(strDdate); arrivalHeader.setSeqNO(strDHMS);
		 * arrival.setHeader(arrivalHeader); ArrivalMessage arrivalMessage=new
		 * ArrivalMessage();
		 * 
		 * ArrivalDataRoot arrivalDataRoot=new ArrivalDataRoot(); List<ArrivalData>
		 * listArrivalData=new ArrayList(); //主表vo ArrayList<DJZBHeaderVO> vos
		 * =(ArrayList<DJZBHeaderVO>) getDao().executeQuery(sql1, new
		 * BeanListProcessor(DJZBHeaderVO.class));
		 * 
		 * //if(strbilltype.equals("F2-01")){ int iRow=0; for(DJZBHeaderVO vo : vos){
		 * String
		 ...
		 * getDao().executeQuery(sql2, new BeanListProcessor(DJZBItemVO.class));
		 ...
		 * +"'"; String currency=(String)getDao().executeQuery(sql3, new
		 * ColumnProcessor());
		 * sql3="select rq from arap_dztz where pk_dztz='"+vob.getDdhh()+"'";
		 * Logger.info("sql rq:"+sql3); String rq=(String)getDao().executeQuery(sql3,
		 * new ColumnProcessor()); //数据data ArrivalData arrivalData=new ArrivalData();
		 ...
		 * arrivalData.setOperatorCode(userCode); arrivalData.setCurrency(currency);
		 * arrivalData.setArrivalRegiCode(vo.getVouchid());
		 * arrivalData.setArrivalDate(rq); listArrivalData.add(arrivalData); }
		 * 
		 * iRow++; }
		 * 
		 * arrivalDataRoot.setTotalNum(iRow);
		 * 
		 * switch(strbilltype){ case "F2-01": arrivalDataRoot.setArrivalBusinType("2");
		 * break; case "F2-02": arrivalDataRoot.setArrivalBusinType("1"); break; }
		 * 
		 * 
		 * 2023-02-15 arrivalBusinType 字符串类型不变 取值范围改变
		 * 
		 * arrivalDataRoot.setArrivalBusinType(strbilltype);
		 * 
		 * arrivalDataRoot.setArrivalData(listArrivalData);
		 * 
		 * //加密 EncryptHelper encryptHelper=new EncryptHelper(); String
		 * encryptString=JSON.toJSONString(arrivalDataRoot);
		 * Logger.info("arrivalDataRoot:"+encryptString); strTemp = encryptString; try {
		 * encryptString = encryptHelper.encrypt(encryptString);
		 * 
		 * } catch (SecurityException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } arrivalMessage.setData(encryptString); //解密 try {
		 * encryptString=encryptHelper.decrypt(encryptString); } catch
		 * (SecurityException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } //httpPost
		 * 
		 ...
		 * u8c.server.XmlConfig.getUrl("ArrivalApi"), JSON.toJSONString(arrival));
		 * JSONObject parameJson = JSONObject.parseObject(strResult); RespMsg
		 * respMsg=JSON.toJavaObject(parameJson, RespMsg.class);
		 * 
		 * try { strResult=encryptHelper.decrypt(respMsg.getMessage().getData());
		 * 
		 * 20220320 回写结果到zyx11,zyx12
		 * 
		 * updateArrival(strResult); } catch (SecurityException e) { // TODO
