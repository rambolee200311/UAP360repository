...
public class arrival implements IAPICustmerDevelop{
	...
	private PostResult setPostResultSKHZ(ConfirmArrivalBody confirmArrivalBody,String strDjlxbm){
		...
		try{
			...
			if (!confirmArrivalBody.getCurrency().equals("CNY")){
				...
				childrenvo.setDfbbje(Double.toString(confirmArrivalBody.getArrivalRMB()*-1));
			}
			childrenvo.setDfybje(Double.toString(confirmArrivalBody.getArrivalAmount()*-1));
	...
	private PostResult setPostResultSKLZ(ConfirmArrivalBody confirmArrivalBody,String strDjlxbm){
			...
			try{
				 ...
				 * +" inner join arap_djfb b on a.pk_cubasdoc=b.hbbm"
				 * +" inner join arap_djzb c on b.vouchid=c.vouchid"
				 * +" where c.vouchid='"+confirmArrivalBody.getArrivalRegiCode()+"'"; String
				 * custcode=""; custcode=(String)getDao().executeQuery(sql, new
				 * ColumnProcessor());
				 */
				custcode=confirmArrivalBody.getApCode();
				//到账认领收款
				BillVO billVO=new BillVO();		
				//单据头
				ParentVO parentvo=new ParentVO();
				...
				if (!confirmArrivalBody.getCurrency().equals("CNY")){
					...
					childrenvo.setDfbbje(Double.toString(confirmArrivalBody.getArrivalRMB()));
				}
				childrenvo.setDfybje(Double.toString(confirmArrivalBody.getArrivalAmount()));
	...
	private PostResult setPostResultYF(ConfirmArrivalBody confirmArrivalBody,String strDjlxbm){
		...
		try{
			...
			if (!confirmArrivalBody.getCurrency().equals("CNY")){
				...
				childrenvo1.setDfbbje(Double.toString(confirmArrivalBody.getArrivalRMB()));
			}
			childrenvo1.setDfybje(Double.toString(confirmArrivalBody.getArrivalAmount()));
			
			childrenvo1.setHbbm(custcode);
